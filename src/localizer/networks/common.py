    ...
    def __init__(
        ...
        if weights_path is not None:
            if weights_path.is_file():
                ...
                logger.info(
                    f"ResNet50CoordsExtractor: Using weights found at '{str(weights_path)}'."
                )
            else:
                logger.info(
                    f"ResNet50CoordsExtractor: No weights found at '{str(weights_path)}'"
                    ", using ImageNet2 wegihts."
                )
        else:
            logger.info("ResNet50CoordsExtractor: Using ImageNet2 weights.")
